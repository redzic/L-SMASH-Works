...
static inline __m128i _MM_PACKUS_EPI32(const __m128i *low,
                                       const __m128i *high) {
...
static void make_black_background_planar_yuv8(VSFrameRef *vs_frame,
                                              const VSAPI *vsapi) {
  for (int i = 0; i < 3; i++)
    memset(vsapi->getWritePtr(vs_frame, i), i ? 0x80 : 0x00,
           vsapi->getStride(vs_frame, i) * vsapi->getFrameHeight(vs_frame, i));
}

static void make_black_background_planar_yuv16(VSFrameRef *vs_frame,
                                               const VSAPI *vsapi) {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    uint8_t *end = data + vsapi->getStride(vs_frame, i) *
                              vsapi->getFrameHeight(vs_frame, i);
    while (data < end) {
...
static void make_black_background_planar_gray(VSFrameRef *vs_frame,
                                              const VSAPI *vsapi) {
  memset(vsapi->getWritePtr(vs_frame, 0), 0x00,
         vsapi->getStride(vs_frame, 0) * vsapi->getFrameHeight(vs_frame, 0));
}

static void make_black_background_planar_rgb(VSFrameRef *vs_frame,
                                             const VSAPI *vsapi) {
  for (int i = 0; i < 3; i++)
    memset(vsapi->getWritePtr(vs_frame, i), 0x00,
           vsapi->getStride(vs_frame, i) * vsapi->getFrameHeight(vs_frame, i));
}

static void make_frame_planar_yuv(lw_video_scaler_handler_t *vshp,
                                  AVFrame *av_picture,
  ...
  vs_picture_t vs_picture = {
      /* data */
      {vsapi->getWritePtr(vs_frame, 0), vsapi->getWritePtr(vs_frame, 1),
       vsapi->getWritePtr(vs_frame, 2), NULL},
      /* linesize */
      {vsapi->getStride(vs_frame, 0), vsapi->getStride(vs_frame, 1),
       vsapi->getStride(vs_frame, 2), 0}};
  if (vshp->input_pixel_format == AV_PIX_FMT_P010LE &&
      vshp->output_pixel_format == AV_PIX_FMT_YUV420P10LE) {
    ...
    for (int y = 0; y < height_uv; y++) {
      for (int x = 0; x < width_uv; x += 8) {
        __m128i uv_low =
            _mm_load_si128((__m128i *)((uint32_t *)srcp_uv + x + 0));
        __m128i uv_high =
            _mm_load_si128((__m128i *)((uint32_t *)srcp_uv + x + 4));
    ...
    sws_scale(vshp->sws_ctx, (const uint8_t *const *)av_picture->data,
              av_picture->linesize, 0, av_picture->height, vs_picture.data,
              vs_picture.linesize);
}

static void make_frame_planar_gray(lw_video_scaler_handler_t *vshp,
                                   AVFrame *av_picture,
                                   const component_reorder_t *component_reorder,
                                   VSFrameRef *vs_frame,
                                   VSFrameContext *frame_ctx,
...
static void make_frame_planar_rgb(lw_video_scaler_handler_t *vshp,
                                  AVFrame *av_picture,
                                  const component_reorder_t *component_reorder,
                                  VSFrameRef *vs_frame,
                                  VSFrameContext *frame_ctx,
...
static void
make_frame_planar_alpha(lw_video_scaler_handler_t *vshp, AVFrame *av_picture,
                        const component_reorder_t *component_reorder,
                        VSFrameRef *vs_frame, VSFrameContext *frame_ctx,
                        const VSAPI *vsapi) {
  vs_bitblt(vsapi->getWritePtr(vs_frame, 0), vsapi->getStride(vs_frame, 0),
            av_picture->data[3], av_picture->linesize[3],
            av_picture->width * vsapi->getFrameFormat(vs_frame)->bytesPerSample,
            av_picture->height);
}

static void
make_frame_planar_alpha8(lw_video_scaler_handler_t *vshp, AVFrame *av_picture,
                         const component_reorder_t *component_reorder,
                         VSFrameRef *vs_frame, VSFrameContext *frame_ctx,
                         const VSAPI *vsapi) {
  ...
  for (int i = 0; i < av_picture->height; i++) {
    uint8_t *av_pixel =
        av_picture->data[0] + av_pixel_offset + component_reorder[3];
...
static void
make_frame_planar_alpha16(lw_video_scaler_handler_t *vshp, AVFrame *av_picture,
                          const component_reorder_t *component_reorder,
                          VSFrameRef *vs_frame, VSFrameContext *frame_ctx,
                          const VSAPI *vsapi) {
  uint8_t *vs_frame_data = vsapi->getWritePtr(vs_frame, 0);
  ...
  for (int i = 0; i < av_picture->height; i++) {
    uint16_t *av_pixel = (uint16_t *)(av_picture->data[0] + av_pixel_offset) +
                         component_reorder_get_order(component_reorder[3]);
    uint16_t *vs_pixel = (uint16_t *)(vs_frame_data + vs_pixel_offset);
    for (int j = 0; j < av_picture->width; j++) {
      *(vs_pixel++) =
          be ? ((*av_pixel >> 8) | ((*av_pixel & 0xff) << 8)) : *av_pixel;
      av_pixel += 4;
    }
    av_pixel_offset += av_picture->linesize[0];
    vs_pixel_offset += vs_frame_linesize;
...
VSPresetFormat get_vs_output_pixel_format(const char *format_name) {
  ...
  } format_table[] = {{"YUV420P8", pfYUV420P8},   {"YUV422P8", pfYUV422P8},
                      {"YUV444P8", pfYUV444P8},   {"YUV410P8", pfYUV410P8},
                      {"YUV411P8", pfYUV411P8},   {"YUV440P8", pfYUV440P8},
                      {"YUV420P9", pfYUV420P9},   {"YUV422P9", pfYUV422P9},
                      {"YUV444P9", pfYUV444P9},   {"YUV420P10", pfYUV420P10},
                      {"YUV422P10", pfYUV422P10}, {"YUV444P10", pfYUV444P10},
                      {"YUV420P12", pfYUV420P12}, {"YUV422P12", pfYUV422P12},
                      {"YUV444P12", pfYUV444P12}, {"YUV420P14", pfYUV420P14},
                      {"YUV422P14", pfYUV422P14}, {"YUV444P14", pfYUV444P14},
                      {"YUV420P16", pfYUV420P16}, {"YUV422P16", pfYUV422P16},
                      {"YUV444P16", pfYUV444P16}, {"Y8", pfGray8},
                      {"Y16", pfGray16},          {"RGB24", pfRGB24},
                      {"RGB27", pfRGB27},         {"RGB30", pfRGB30},
                      {"RGB48", pfRGB48},         {NULL, pfNone}};
...
vs_to_av_output_pixel_format(VSPresetFormat vs_output_pixel_format) {
  static const struct {
    VSPresetFormat vs_output_pixel_format;
    enum AVPixelFormat av_output_pixel_format;
  } format_table[] = {{pfYUV420P8, AV_PIX_FMT_YUV420P},
...
static const component_reorder_t *
get_component_reorder(enum AVPixelFormat av_output_pixel_format) {
  static const struct {
    enum AVPixelFormat av_output_pixel_format;
...
static inline int set_frame_maker(vs_video_output_handler_t *vs_vohp,
  ...
  static const struct {
    ...
    func_make_frame *func_make_frame;
      ...
      {pfYUV420P9, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV422P9, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV444P9, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV420P10, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV422P10, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV444P10, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV420P12, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV422P12, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV444P12, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV420P14, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV422P14, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV444P14, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV420P16, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV422P16, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
      {pfYUV444P16, 0, make_black_background_planar_yuv16,
       make_frame_planar_yuv},
  ...
  for (int i = 0; frame_maker_table[i].vs_output_pixel_format != pfNone; i++)
    if (vs_vohp->vs_output_pixel_format ==
            frame_maker_table[i].vs_output_pixel_format &&
        output_index == frame_maker_table[i].output_index) {
      vs_vohp->make_black_background[output_index] =
          frame_maker_table[i].func_make_black_background;
...
static int
determine_colorspace_conversion(vs_video_output_handler_t *vs_vohp,
                                int output_index,
  ...
  } conversion_table[] = {{AV_PIX_FMT_YUV420P, pfYUV420P8, 0},
  ...
  if (vs_vohp->variable_info || vs_vohp->vs_output_pixel_format == pfNone) {
    ...
    for (int i = 0; conversion_table[i].vs_output_pixel_format != pfNone; i++)
      if (input_pixel_format == conversion_table[i].av_input_pixel_format) {
        vs_vohp->vs_output_pixel_format =
            conversion_table[i].vs_output_pixel_format;
  ...
  *output_pixel_format =
  ...
  if (*output_pixel_format == AV_PIX_FMT_NONE)
    return -1;
  vs_vohp->component_reorder[output_index] = get_component_reorder(
      output_index ? input_pixel_format : *output_pixel_format);
...
static VSFrameRef *
new_output_video_frame(vs_video_output_handler_t *vs_vohp,
                       const AVFrame *av_frame, int output_index,
                       enum AVPixelFormat *output_pixel_format,
                       int input_pix_fmt_change, VSFrameContext *frame_ctx,
                       VSCore *core, const VSAPI *vsapi) {
  if (vs_vohp->variable_info) {
    if (!av_frame->opaque &&
        determine_colorspace_conversion(vs_vohp, output_index, av_frame->format,
                                        output_pixel_format) < 0)
      goto fail;
    const VSFormat *vs_format =
        vsapi->getFormatPreset(vs_vohp->vs_output_pixel_format, core);
    return vsapi->newVideoFrame(vs_format, av_frame->width, av_frame->height,
                                NULL, core);
  } else {
...
fail:
  if (frame_ctx)
    vsapi->setFilterError("lsmas: failed to determine colorspace conversion.",
                          frame_ctx);
  return NULL;
}

VSFrameRef *make_frame(lw_video_output_handler_t *vohp, AVFrame *av_frame,
                       int output_index) {
  vs_video_output_handler_t *vs_vohp =
      (vs_video_output_handler_t *)vohp->private_handler;
  ...
  if (av_frame->opaque) {
    ...
    vs_video_buffer_handler_t *vs_vbhp =
        (vs_video_buffer_handler_t *)av_frame->opaque;
    return vs_vbhp ? (VSFrameRef *)vs_vbhp->vsapi->cloneFrameRef(
                         vs_vbhp->vs_frame_buffer)
                   : NULL;
  }
  /* Make video frame.
   * Convert pixel format if needed. We don't change the presentation
   * resolution. */
  VSFrameRef *vs_frame = new_output_video_frame(
      vs_vohp, av_frame, output_index, &vshp->output_pixel_format,
      !!(vshp->frame_prop_change_flags &
         LW_FRAME_PROP_CHANGE_FLAG_PIXEL_FORMAT),
      frame_ctx, core, vsapi);
  if (!vs_vohp->make_frame[output_index])
    return NULL;
  if (vs_frame)
    vs_vohp->make_frame[output_index](vshp, av_frame,
                                      vs_vohp->component_reorder[output_index],
                                      vs_frame, frame_ctx, vsapi);
  else if (frame_ctx)
    vsapi->setFilterError("lsmas: failed to allocate a output video frame.",
                          frame_ctx);
  return vs_frame;
}

static int vs_check_dr_available(AVCodecContext *ctx,
  ...
  static enum AVPixelFormat dr_support_pix_fmt[] = {
      ...
      AV_PIX_FMT_GRAY8,       AV_PIX_FMT_GRAY16LE,    AV_PIX_FMT_GBRP,
      AV_PIX_FMT_GBRP9LE,     AV_PIX_FMT_GBRP10LE,    AV_PIX_FMT_GBRP16LE,
      AV_PIX_FMT_NONE};
...
static void vs_video_unref_buffer_handler(void *opaque, uint8_t *data) {
   ...
   * Delete it by vs_video_release_buffer_handler() if there are no reference to
   * it i.e. the reference-counter equals zero. */
  AVBufferRef *vs_buffer_ref = (AVBufferRef *)opaque;
  av_buffer_unref(&vs_buffer_ref);
}

static inline int vs_create_plane_buffer(vs_video_buffer_handler_t *vs_vbhp,
                                         AVBufferRef *vs_buffer_handler,
                                         AVFrame *av_frame, int av_plane,
                                         int vs_plane) {
  ...
  av_frame->linesize[av_plane] =
      vs_vbhp->vsapi->getStride(vs_vbhp->vs_frame_buffer, vs_plane);
  int vs_plane_size =
      vs_vbhp->vsapi->getFrameHeight(vs_vbhp->vs_frame_buffer, vs_plane) *
      av_frame->linesize[av_plane];
  av_frame->buf[av_plane] = av_buffer_create(
      vs_vbhp->vsapi->getWritePtr(vs_vbhp->vs_frame_buffer, vs_plane),
      vs_plane_size, vs_video_unref_buffer_handler, vs_buffer_ref, 0);
...
static int vs_video_get_buffer(AVCodecContext *ctx, AVFrame *av_frame,
                               int flags) {
  av_frame->opaque = NULL;
  lw_video_output_handler_t *lw_vohp = (lw_video_output_handler_t *)ctx->opaque;
  vs_video_output_handler_t *vs_vohp =
      (vs_video_output_handler_t *)lw_vohp->private_handler;
  ...
  if ((!vs_vohp->variable_info &&
       lw_vohp->scaler.output_pixel_format != pix_fmt) ||
      !vs_check_dr_available(ctx, pix_fmt))
    return avcodec_default_get_buffer2(ctx, av_frame, flags);
  /* New VapourSynth video frame buffer. */
  vs_video_buffer_handler_t *vs_vbhp =
      (vs_video_buffer_handler_t *)malloc(sizeof(vs_video_buffer_handler_t));
  if (!vs_vbhp) {
    av_frame_unref(av_frame);
    return AVERROR(ENOMEM);
  }
  av_frame->opaque = vs_vbhp;
  avcodec_align_dimensions2(ctx, &av_frame->width, &av_frame->height,
                            av_frame->linesize);
  VSFrameRef *vs_frame_buffer =
      new_output_video_frame(vs_vohp, av_frame, 0, NULL, 0, vs_vohp->frame_ctx,
                             vs_vohp->core, vs_vohp->vsapi);
  if (!vs_frame_buffer) {
    free(vs_vbhp);
    av_frame_unref(av_frame);
    return AVERROR(ENOMEM);
   ...
   * The callback vs_video_release_buffer_handler() shall be called when no
   * reference to the video buffer handler is present. The callback
   * vs_video_unref_buffer_handler() decrements the reference-counter by 1. */
  ...
  AVBufferRef *vs_buffer_handler =
      av_buffer_create(NULL, 0, vs_video_release_buffer_handler, vs_vbhp, 0);
  ...
  for (int i = 0; i < 3; i++)
    if (vs_create_plane_buffer(vs_vbhp, vs_buffer_handler, av_frame, i,
                               vs_vohp->component_reorder[0][i]) < 0)
      goto fail;
  /* Here, a variable 'vs_buffer_handler' itself is not referenced by any
   * pointer. */
  av_buffer_unref(&vs_buffer_handler);
...
int vs_setup_video_rendering(lw_video_output_handler_t *lw_vohp,
                             AVCodecContext *ctx, VSVideoInfo *vi, VSMap *out,
                             int width, int height) {
  vs_video_output_handler_t *vs_vohp =
      (vs_video_output_handler_t *)lw_vohp->private_handler;
  const VSAPI *vsapi = vs_vohp->vsapi;
  enum AVPixelFormat output_pixel_format, alpha_pixel_format;
  if (determine_colorspace_conversion(vs_vohp, 0, ctx->pix_fmt,
                                      &output_pixel_format)) {
    set_error_on_init(out, vsapi, "lsmas: %s is not supported",
                      av_get_pix_fmt_name(ctx->pix_fmt));
    return -1;
  }
  if (av_pix_fmt_desc_get(ctx->pix_fmt)->flags & AV_PIX_FMT_FLAG_ALPHA &&
      determine_colorspace_conversion(vs_vohp, 1, ctx->pix_fmt,
                                      &alpha_pixel_format)) {
    set_error_on_init(out, vsapi, "lsmas: %s's alpha format is not supported",
                      av_get_pix_fmt_name(ctx->pix_fmt));
    return -1;
  }
  vs_vohp->direct_rendering &= vs_check_dr_available(ctx, ctx->pix_fmt);
  int (*dr_get_buffer)(struct AVCodecContext *, AVFrame *, int) =
      vs_vohp->direct_rendering ? vs_video_get_buffer : NULL;
  setup_video_rendering(lw_vohp, SWS_FAST_BILINEAR, width, height,
                        output_pixel_format, ctx, dr_get_buffer);
  if (vs_vohp->variable_info) {
    vi->format = NULL;
    vi->width = 0;
    vi->height = 0;
    ...
    vi->format =
        vsapi->getFormatPreset(vs_vohp->vs_output_pixel_format, vs_vohp->core);
    vi->width = lw_vohp->output_width;
    vi->height = lw_vohp->output_height;
    vs_vohp->background_frame[0] = vsapi->newVideoFrame(
        vi->format, vi->width, vi->height, NULL, vs_vohp->core);
    if (!vs_vohp->background_frame[0]) {
      set_error_on_init(out, vsapi,
                        "lsmas: failed to allocate memory for the background "
                        "black frame data.");
      return -1;
    }
...
static void vs_free_video_output_handler(void *private_handler) {
  vs_video_output_handler_t *vs_vohp =
      (vs_video_output_handler_t *)private_handler;
  if (!vs_vohp)
    return;
...
vs_allocate_video_output_handler(lw_video_output_handler_t *vohp) {
  vs_video_output_handler_t *vs_vohp =
      (vs_video_output_handler_t *)lw_malloc_zero(
          sizeof(vs_video_output_handler_t));
  if (!vs_vohp)
    return NULL;
...
void vs_set_frame_properties(AVFrame *av_frame, AVStream *stream,
                             int64_t duration_num, int64_t duration_den,
                             VSFrameRef *vs_frame, int top, int bottom,
                             const VSAPI *vsapi) {
  VSMap *props = vsapi->getFramePropsRW(vs_frame);
  /* Sample aspect ratio */
  vsapi->propSetInt(props, "_SARNum", av_frame->sample_aspect_ratio.num,
                    paReplace);
  vsapi->propSetInt(props, "_SARDen", av_frame->sample_aspect_ratio.den,
                    paReplace);
  /* Sample duration */
  vsapi->propSetInt(props, "_DurationNum", duration_num, paReplace);
  vsapi->propSetInt(props, "_DurationDen", duration_den, paReplace);
  /* Color format
   * The decoded color format may not match with the output. Set proper
   * properties when no YUV->RGB conversion is there. */
  ...
  if (vs_format->colorFamily != cmRGB) {
    if (av_frame->color_range != AVCOL_RANGE_UNSPECIFIED)
      vsapi->propSetInt(props, "_ColorRange",
                        av_frame->color_range == AVCOL_RANGE_MPEG, paReplace);
    vsapi->propSetInt(props, "_Primaries", av_frame->color_primaries,
                      paReplace);
    ...
    if (av_frame->chroma_location > 0)
      vsapi->propSetInt(props, "_ChromaLocation", av_frame->chroma_location - 1,
                        paReplace);
  }
  /* Picture type */
  ...
  const AVFrameSideData *mastering_display_side_data = av_frame_get_side_data(
      av_frame, AV_FRAME_DATA_MASTERING_DISPLAY_METADATA);
  if (mastering_display_side_data) {
    const AVMasteringDisplayMetadata *mastering_display =
        (const AVMasteringDisplayMetadata *)mastering_display_side_data->data;
    if ((frame_has_primaries = mastering_display->has_primaries)) {
      ...
      for (int i = 0; i < 3; i++) {
        display_primaries_x[i] =
            av_q2d(mastering_display->display_primaries[i][0]);
        display_primaries_y[i] =
            av_q2d(mastering_display->display_primaries[i][1]);
      }
      vsapi->propSetFloatArray(props, "MasteringDisplayPrimariesX",
                               display_primaries_x, 3);
      vsapi->propSetFloatArray(props, "MasteringDisplayPrimariesY",
                               display_primaries_y, 3);
      vsapi->propSetFloat(props, "MasteringDisplayWhitePointX",
                          av_q2d(mastering_display->white_point[0]), paReplace);
      vsapi->propSetFloat(props, "MasteringDisplayWhitePointY",
                          av_q2d(mastering_display->white_point[1]), paReplace);
    }
    if ((frame_has_luminance = mastering_display->has_luminance)) {
      vsapi->propSetFloat(props, "MasteringDisplayMinLuminance",
                          av_q2d(mastering_display->min_luminance), paReplace);
      vsapi->propSetFloat(props, "MasteringDisplayMaxLuminance",
                          av_q2d(mastering_display->max_luminance), paReplace);
    }
  }
  ...
  const AVFrameSideData *content_light_side_data =
      av_frame_get_side_data(av_frame, AV_FRAME_DATA_CONTENT_LIGHT_LEVEL);
  if (content_light_side_data) {
    ...
    if ((frame_has_light_level =
             content_light->MaxCLL || content_light->MaxFALL)) {
      vsapi->propSetInt(props, "ContentLightLevelMax", content_light->MaxCLL,
                        paReplace);
      vsapi->propSetInt(props, "ContentLightLevelAverage",
                        content_light->MaxFALL, paReplace);
    }
  }
  if (stream && !frame_has_light_level) {
    for (int i = 0; i < stream->nb_side_data; i++) {
      if (stream->side_data[i].type == AV_PKT_DATA_CONTENT_LIGHT_LEVEL) {
        const AVContentLightMetadata *content_light =
            (const AVContentLightMetadata *)stream->side_data[i].data;
        if (content_light->MaxCLL || content_light->MaxFALL) {
          vsapi->propSetInt(props, "ContentLightLevelMax",
                            content_light->MaxCLL, paReplace);
          vsapi->propSetInt(props, "ContentLightLevelAverage",
                            content_light->MaxFALL, paReplace);
        }
        break;
  ...
  // Dobly RPU
  const AVFrameSideData *rpu_side_data =
      av_frame_get_side_data(av_frame, AV_FRAME_DATA_DOVI_RPU_BUFFER);
  if (rpu_side_data && rpu_side_data->size > 0) {
    vsapi->propSetData(props, "DolbyVisionRPU",
                       (const char *)rpu_side_data->data, rpu_side_data->size,
                       paReplace);
  }
#endif
}
